...
lm_summarize = function(lm,
  ...
  if((any(attr(lm$terms,"dataClasses")=="factor") | any(attr(lm$terms,"dataClasses")=="factor")) &
     (std | semi | partial) & warn) warning("Dani messed up...don't trust the betas or semi/partial correlations reported for 'factor' (nominal) or 'ordered factor' (ordinal) variables.\nThe values calculated are for when those varaibles are transformed to numeric variables, not what R calculates by default for thes types of variables...\n...He'll fix this soon...",
                                     immediate. = T)
  # print(lm$call)
